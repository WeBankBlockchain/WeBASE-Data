...
package data.collect.test.method;
...
import data.collect.test.TestBase;

/**
 * 配置sdk的encryptType，配置为国密或标密
 */
public class MethodIdTest extends TestBase {
    ...
    public void getAllPreMethodId() throws IOException {
        ...
        List<AbiDefinition> gasChargeManage = Web3Tools.loadContractDefinition(gas_charge_manage_abi);
        ...
        System.out.println("gasChargeManage");
        for (AbiDefinition abiDefinition : gasChargeManage) {
            if ("function".equals(abiDefinition.getType())) {
                // support guomi sm3
                String buildMethodId = Web3Tools.buildMethodId(abiDefinition);
                System.out.println(abiDefinition.getName() + ": " + buildMethodId);
