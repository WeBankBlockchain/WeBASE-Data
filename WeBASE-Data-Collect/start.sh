...
APP_MAIN=com.webank.webase.data.collect.Application
...
start(){
    ...
    if [ $processStatus == 1 ]; then
        ...
        nohup $JAVA_HOME/bin/java -Djdk.tls.namedGroups="secp256k1" $JAVA_OPTS -cp $CLASSPATH $APP_MAIN >> $LOG_DIR/server.out 2>&1 &
       ...
       if [ $result -ne 0 ]; then
           echo "PID($processPid) [Starting]. Please message check through the log file (default path:./log/)."
           echo "==============================================================================================="
       else
